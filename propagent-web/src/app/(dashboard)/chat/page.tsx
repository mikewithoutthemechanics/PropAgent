...
export default function ChatPage() {
  ...
  return (
    <div className="space-y-6 h-[calc(100vh-180px)]">
      <div className="bg-gradient-to-r from-lime-400 to-sky-400 bg-clip-text text-transparent">
        <h1 className="text-2xl font-bold text-charcoal-900">AI Communication</h1>
        <p className="text-charcoal-500 mt-1">AI-powered tenant communication hub</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-full">
        <Card className="lg:col-span-1 flex flex-col bg-white border-2 border-charcoal-100 rounded-2xl">
          <CardHeader 
            title="Conversations" 
            subtitle={`${totalUnread} unread`}
            action={
              <Button onClick={() => setShowNewChatModal(true)} className="text-xs px-2 py-1 rounded-full">
                <Plus className="w-3 h-3 mr-1" />
                New
              </Button>
            }
          />
          <div className="flex-1 overflow-y-auto divide-y divide-charcoal-100">
            {conversations.map((conv) => (
              <button
                key={conv.id}
                onClick={() => setActiveConversationId(conv.id)}
                className={`w-full p-4 text-left hover:bg-charcoal-50 transition-colors duration-200 cursor-pointer ${
                  activeConversationId === conv.id ? 'bg-lime-50' : ''
                }`}
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      <p className="font-medium text-charcoal-900 truncate">{conv.tenantName}</p>
                      {conv.unreadCount > 0 && (
                        <span className="bg-lime-400 text-charcoal-900 text-xs font-medium px-1.5 py-0.5 rounded-full">
                          {conv.unreadCount}
                        </span>
                      )}
                    </div>
                    <p className="text-sm text-charcoal-500 truncate">{conv.lastMessage}</p>
                    <p className="text-xs text-charcoal-500 mt-1">{formatDateTime(conv.lastMessageTime)}</p>
                  </div>
                </div>
              </button>
            ))}
        ...
        <Card className="lg:col-span-2 flex flex-col bg-white border-2 border-charcoal-100 rounded-2xl">
          {activeConversation ? (
            <>
              <div className="p-4 border-b border-charcoal-100">
                <div className="flex items-center gap-3">
                  ...
                  />
                  <div>
                    <p className="font-semibold text-charcoal-900">{activeConversation.tenantName}</p>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                    ...
                    <div className={`max-w-[70%] ${message.senderType === 'ai' ? 'order-2' : ''}`}>
                      {message.senderType === 'ai' && (
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className="text-xs font-medium text-lime-600">PropAgent AI</span>
                        </div>
                      )}
                      {message.senderType === 'tenant' && (
                        <div className="flex items-center gap-2 mb-1">
                          <User className="w-3 h-3 text-charcoal-500" />
                          <span className="text-xs text-charcoal-500">{activeConversation.tenantName}</span>
                        </div>
                      )}
                      ...
                      <p className={`text-xs text-charcoal-500 mt-1 ${message.senderType === 'agent' || message.senderType === 'landlord' ? 'text-right' : ''}`}>
                ...
                {isTyping && (
                  <div className="flex items-center gap-2">
                    ...
                    <div className="bg-charcoal-100 rounded-2xl rounded-bl-md px-4 py-3">
              ...
              <div className="p-4 border-t border-charcoal-100">
                <div className="flex items-center gap-3">
                  <input
                    ...
                    className="flex-1 px-4 py-3 bg-charcoal-50 border border-charcoal-200 rounded-lg text-charcoal-900 placeholder:text-charcoal-500 focus:outline-none focus:ring-2 focus:ring-lime-400/20 focus:border-lime-400 transition-all duration-200"
                  />
            ...
            <div className="flex-1 flex flex-col items-center justify-center">
              <MessageSquare className="w-12 h-12 text-charcoal-500 mb-4" />
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 border-2 border-charcoal-100">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-xl font-semibold text-charcoal-900">New Chat</h2>
              <button 
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-charcoal-500 mb-2">Tenant Name</label>
                <input 
                  type="text" 
              ...
              <div>
                <label className="block text-sm font-medium text-charcoal-500 mb-2">Initial Message (optional)</label>
                <textarea 
                  rows={3}
            ...
            <div className="flex items-center justify-end gap-3 mt-6">
              <button 
                onClick={() => setShowNewChatModal(false)}
                className="px-4 py-2.5 bg-charcoal-50 rounded-full text-charcoal-900 text-sm hover:bg-charcoal-200 transition-all cursor-pointer"
              >
                Cancel
              </button>
              <button 
                ...
                className="px-6 py-2.5 bg-lime-400 rounded-full text-charcoal-900 text-sm font-medium hover:shadow-lg hover:shadow-lime-400/25 transition-all disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
              >
